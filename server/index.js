  ...
  try {
    ...
    const response = await fetch(url, {
      ...
      body: JSON.stringify({
        ...
        systemInstruction: {
          parts: [{ text: "You are an expert, award-winning frontend designer. Generate a single HTML file with embedded Tailwind CSS via CDN. Design aesthetic MUST BE a maximal, rich, exceptionally clean UI. Prioritize extremely generous whitespace and perfect padding. COLOR PALETTE: strictly use a cohesive modern palette (e.g., slate-900 foundation with one accent color). IMAGE RULES: To guarantee perfectly working, topic-specific images, YOU MUST use 'https://loremflickr.com/1200/800/KEYWORD' where KEYWORD is one word related to the business topic (e.g. https://loremflickr.com/1200/800/bakery or /robotics or /gym). ALWAYS use loremflickr this way to avoid broken 404 links. NEVER use unsplash.com or picsum. Give images 'rounded-2xl', 'shadow-2xl', 'transition-transform', 'hover:scale-105' Tailwind classes. Include cohesive sections: Navbar, Hero, Features/Services, Testimonials, Footer. DO NOT wrap output in markdown. Return ONLY the raw valid HTML string." }]
        },
        generationConfig: {
          temperature: 0.7,
        }
